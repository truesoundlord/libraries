...
int main(int argc,char *argv[])
{
	...
	sigaction(SIGALRM,&detournement,NULL);				// kill -14
	...
	// Normalement le "shell" rétabli le comportement standard des signaux
	// à la sortie du programme... nous n'avons pas besoin de refaire
	// un toggleSignalsEcho() ici...
...
void ArretPropre(int signal)
{
	...
	switch(signal)
	{
		...
		case SIGABRT:
									break;
		case SIGPIPE:	fprintf(stderr,"SIGPIPE !!\n");
									break;
		case SIGSEGV: 
		...
		case SIGCONT: fprintf(stderr,"...continue");
									break;
		case SIGALRM: fprintf(stderr,"ALARME !!\n");
									break;
	}
	//toggleSignalsEcho();
}
