...
fwprintf(stderr,L"[DEBUG] %08x (%d)\n",pMsg,taillereelle+1);	
	...
	#ifdef __DEBUG__
	//sprintf(strDebug,"[DEBUG][%-*ls]\n",taillereelle,tmpMsg);
	//fprintf(stderr,strDebug);
	//swprintf(strDebug,taillereelle,"%-*ls",taillereelle,tmpMsg);
...
struct FakeWindowPos DrawBox(int posX,int posY,int height,int length,int color)
{
	...
	for(posX+=1;posX<=origineX+length+1;posX++)
	{
		wprintf(L"\x1b[%d;%dH",posY,posX);
	...
	wprintf(L"\x1b[0m");
	fflush(stdout);
	return (struct FakeWindowPos){origineX,origineY,length-1,height,origineX+1,origineY+1,height+3};
}
...
struct FakeWindowPos DrawTitledBox(int posX,int posY,int height,int length,int color,char *pTitle)
{
	...
	wprintf(L"\x1b[u");							/* restaure la position sauvegardée */
	fflush(stdout);
	return (struct FakeWindowPos){origineX,origineY,length,height,origineX+1,origineY+1,height+3};
}
...
struct FakeWindowPos DrawTitledBoxWithRGB(int posX,int posY,int height,int length,struct s_RGB color,char *pTitle,struct s_RGB titlecolor,short design)
{
	...
	posX+=wcslen(tmpTitle)+2;														// nous devons déplacer le curseur de deux caractères puis la taille du titre...
	for(;posX<origineX+length+1;posX++)
	{
		wprintf(L"\x1b[%d;%dH",posY,posX);
		wprintf(L"%s%lc",pColorBoxString,UTF8_LIGNE);
	}
...
void AddToMessageBoxEx(char *pMessage,struct FakeWindowPos datas,unsigned short *pCurrentNumber)
{
	...
	//wchar_t *tmpString=malloc(datas.Width*sizeof(wchar_t));
...
// Sortie: néant
//*****************************************************************************
void InitCSSMWIDE(char *pLocale)
{
 ...
 * 
 * Output: un identifiant de thread (pour pouvoir effectuer un on/off concernant le défilement)
*/

int DisplayAnimatedText(PositionCadre position,char *pTexte,struct s_RGB Foreground,struct s_RGB Background)
{
	int local_threadID;
...
void fct_core(void *magie)
{
  ...
  tmpMsg[taillereelle+1]=L'\0';                     // pour éviter les "effets de bord"
	...
	//pthread_mutex_init(&fctcore_mtx_protection,NULL);
  ...
  while(fctcore_brunning)
  {
    ...
    while(indicelettre<taillereelle-1)							// Nous devons lire tout le texte...
    {
      ...
      wprintf(L"\x1b[%d;%dH%lc",donnees->data_posCadre.FirstPrintableY,donnees->data_posCadre.FirstPrintableX+posX,tmpMsg[indicelettre]);
      ...
      if(posX>donnees->data_posCadre.Width)         // ...si nous arrivons à la fin du cadre...
      {
        posX=0;
        indicelettre-=donnees->data_posCadre.Width;                                                     // donne l'effet de défilement...
    ...
    posX=donnees->data_posCadre.Width+donnees->data_posCadre.FirstPrintableX;            // on doit commencer non plus à la première colonne disponible DANS le cadre mais à la dernière...
    ...
    while(indicelettre>=0)
    {
      ...
      wprintf(L"%lc",tmpMsg[indicelettre]); 
			//wprintf(L"\x1b[%d;%dH%lc",donnees->data_posCadre.FirstPrintableY,posX,tmpMsg[indicelettre]);
      ...
      if(posX<donnees->data_posCadre.FirstPrintableX && indicelettre!=0) // si nous arrivons à la marge de gauche... et qu'en plus nous ne sommes pas arrivés au "début" de la phrase...
      {
        ...
        posX=donnees->data_posCadre.Width+donnees->data_posCadre.FirstPrintableX;
        indicelettre+=donnees->data_posCadre.Width;
        SetCursor(posX,donnees->data_posCadre.FirstPrintableY);
        nanosleep(&attente,NULL);
      }
      indicelettre--;
	...
	if(donnees->pTexte) free(donnees);
	//pthread_mutex_destroy(&fctcore_mtx_protection);
	pthread_exit(0);
}
